...
# Data
X_train = np.load('./Training/X_train_cfd.npy')
X_train_numpy = X_train[:,1:202]-5*X_train[:,0].reshape([X_train.shape[0],1])
X_train_numpy2 = X_train_numpy[0:N] # for 2k training examples 
X_train = torch.tensor(X_train_numpy2,dtype=float)
X_val = np.load('./Validation/X_valid_cfd.npy')
X_val_numpy = X_val[:,1:202]-5*X_val[:,0].reshape([X_val.shape[0],1])
X_val_numpy2 = X_val_numpy[0:N] # for 2k training examples 
X_val = torch.tensor(X_val_numpy2,dtype=float)
d_alt = X_train.shape[1]
print('d_x =', d_alt)

Y_train = np.load('./Training/Y_train_cfd.npy')
Y_train = torch.tensor(np.log(Y_train[0:N]),dtype=float)
Y_val = np.load('./Validation/Y_valid_cfd.npy')
...
for i in range(n_models):
    ...
    torch.save(model.state_dict(), folder_exp+'qrnn_cfd_'+str(i)) # Save trained model
